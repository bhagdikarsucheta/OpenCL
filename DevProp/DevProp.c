...
void printDeviceProperties(void)
{
	...
	{
		printf("There is no OpenCL Device supported Failed. Exitting Now...\n");
		exit(EXIT_FAILURE);
	}
	else
	{
		...
		for (int i = 0; i < dev_count; i++)
		{
			...
			clGetDeviceInfo(ocl_device_ids[i], CL_DEVICE_IMAGE2D_MAX_WIDTH, sizeof(size_t), &szMaxDims[1], NULL);
			printf("GPU Device Supported 2-D Image W X H	:%u X %u Bytes\n", (unsigned int)szMaxDims[0], (unsigned int)szMaxDims[1]);


			clGetDeviceInfo(ocl_device_ids[i], CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(size_t), &szMaxDims[2], NULL);
			clGetDeviceInfo(ocl_device_ids[i], CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(size_t), &szMaxDims[3], NULL);
			clGetDeviceInfo(ocl_device_ids[i], CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(size_t), &szMaxDims[4], NULL);
